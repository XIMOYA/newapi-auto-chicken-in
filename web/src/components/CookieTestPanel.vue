...
NewAPI Cookie 与 GitHub Cookie 由父页面分别挂载本组件，各自维护选择与结果。
-->
<template>
...
const modeLabel = computed(() => props.mode === 'github_cookie' ? 'GitHub Cookie' : 'NewAPI Cookie')
const hint = computed(() => props.mode === 'github_cookie'
  ? '只执行站点 OAuth state 与 GitHub authorize 检查，取得授权 code 即视为凭据可用，不会调用站点 OAuth 回调或执行签到。'
  : '参考 NewAPI Cookie 检测流程：直接验证 /api/user/self；检测到 new_api_refresh 时会先刷新凭据再验证，不会执行签到。')
