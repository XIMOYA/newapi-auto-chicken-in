...
| last_error | string | 任务级错误。当前实现从不写非空值，恒 `""` |
...
`source` 可省略（只用于界面展示「谁在跑」）。重复调用视为重新开始。
...
**仅 API Key**。收尾，立即解锁。**200** `{ "ok": true }`，幂等，重复调用无害。
