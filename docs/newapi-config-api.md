...
**签发全程走代理，不让服务器真实 IP 直接碰 GitHub OAuth：**
- 账号配了自带代理（`proxy`）→ 用它（支持 `http://` 与 `socks5://`）
- 没自带 → 从代理池领一个可用出口
- 代理池也空 → 退回服务器直连（不因没代理而阻断签发，但那一次会暴露服务器 IP）

⚠ 免费机房代理常被 GitHub 的 OAuth 端点限流（403/429）。走代理隐藏了服务器 IP，
但成功率可能不如直连；失败文案里的「当前出口被 GitHub 限制」就是这种情况，重试或
刷新代理池换个出口即可。
