...
服务端实现上有一条不变量值得单独记：**`Set-Cookie` 里有没有 `new_api_refresh`，是判断「源站是否真的处理了这一发 refresh」的唯一可靠判据** —— 只有源站会下发它，CDN 的挑战页不会。所以 `checkTabiAICookie` 在 `client.Do` 返回后立刻抠出这个值，之后所有返回路径都带着它，并且抠到值时当场把内部的 `retryable` 关掉：代次已经推进，这一轮手里的旧代快照就是废纸，拿它换出口重试正是重放特征。

`client.Do` 直接报错时连 header 都没有，只能按错误类型分流：拨号阶段失败（代理连不通、DNS 失败、`proxyconnect`）说明请求从未到达源站、代次不可能推进，可以换代理重试；超时或连接中途断开按最坏情况处理，本轮放弃。
...
1. **签到收尾**（`checkin.yml`）—— 回传的是「这一轮实际用过的代理表现如何」，样本只覆盖被分配到的那些。
2. **代理体检**（`proxy-sweep.yml`，北京 03:00）—— 拉 `GET /api/proxies/available` 把存活代理全测一遍再回传，样本覆盖全量。入口是 `main.py --proxy-sweep --proxy-sweep-minutes N`（默认 50 分钟时间盒）。

体检这一趟单独存在的理由是**出口不同**：平台自己的 `POST /api/proxies/refresh` 和 `/speedtest` 走的是服务器出口，而签到跑在 GitHub Actions 上。代理商封机房 IP 段是常事，服务器那边通的代理到了 Actions 手里可能全是废的 —— 光靠平台自测选出来的「最优」名不副实，签到会一上来连吃几个死代理。体检排在签到前一小时，就是让优选顺序先反映「Actions 用起来好不好」。

体检只测连通性（打 `proxy_pool.test_url`，不碰目标站点），所以只会写 `ok` 和 `net_fail`，`block_fail` 恒为 0 —— 那一栏的语义是「连得上但被目标站拦」，只有真正签到时才判得出来。

时间盒到点没轮到的代理**不记账**：把没测的记成失败会诬陷好代理，反而把排序搞坏，比不体检更糟。
...
Python 侧默认按 `config_sync.url` 同源推导该地址；也可用 `config_sync.writeback_url` 显式指定（支持 `{name}` 占位符）。

**客户端认的是响应体里的 `ok: true`，不是 HTTP 状态码。** 中间要是有网关、鉴权代理或缓存层替服务端回了 200，请求压根没到库里，平台仍然攥着旧代次 —— 只看状态码就会把这种情况当成同步成功。所以自建反代时务必让这个端点的响应体原样透传，不要包一层信封。

失败会**重试 3 次**（退避 1s / 2s）。这条链路打的是配置管理平台而不是被盾挡着的目标站点，重试没有加重风控的顾虑，而漏掉一次的代价是整条会话。
...
设计背景、暂停/恢复规则、代理策略与排查指引见根目录 `README.md` 的「凭据保活与额度报告」一节。
