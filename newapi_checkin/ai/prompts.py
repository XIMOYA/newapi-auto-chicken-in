...
GRID_PROMPT = """这是一张网页截图，宽 {width} 像素，高 {height} 像素，左上角是 (0,0)。
图中**可能**存在点选式验证码，也可能没有。

如果存在，题目要求是：{target}
请依次给出需要点击的每个位置的中心点。

以下情况都属于「没有点选题」，必须按下面的格式回 found=false：
- 只有一个「确认您是真人 / verify you are human」之类的复选框（这种不需要点选图块）
- 只是普通页面、仪表盘、加载中或报错页
- 有验证码但不是点选图块形式（例如输入字符、滑块）

只输出一个 JSON 对象，不要任何解释、不要 markdown 围栏、不要说明你的判断理由：
